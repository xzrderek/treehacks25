...
from tinyagent.src.tiny_agent.computer import Computer
from tinyagent.src.tiny_agent.config import App
from tinyagent.src.tiny_agent.models import (
    ComposeEmailMode,
    NotesMode,
    TinyAgentToolName,
    TransportationOptions,
)
from tinyagent.src.tiny_agent.sub_agents.compose_email_agent import ComposeEmailAgent
from tinyagent.src.tiny_agent.sub_agents.notes_agent import NotesAgent
from tinyagent.src.tiny_agent.sub_agents.pdf_summarizer_agent import PDFSummarizerAgent
from tinyagent.src.tiny_agent.sub_agents.sonar_agent import SonarAgent
from tinyagent.src.tiny_agent.tools.zoom import Zoom
from tinyagent.src.tools.base import StructuredTool, Tool
...
def get_create_calendar_event_tool(computer: Computer) -> Tool:
    async def create_calendar_event(
        ...
        if end_date_args < start_date_args:
            start_date_args = datetime.datetime.fromisoformat(start_date)
            end_date_args = start_date_args + datetime.timedelta(hours=1)
...
def get_read_calendar_tool(computer: Computer) -> Tool:
    async def read_calendar() -> str:
        """Gets the next meeting's end time from the calendar."""
        meeting_info = computer.calendar.get_next_meeting()
        
        if "error" in meeting_info:
            return meeting_info["error"]
            
        return meeting_info["end_date"]

    return Tool(
        name=TinyAgentToolName.READ_CALENDAR.value,
        func=read_calendar,
        description=(
            f"{TinyAgentToolName.READ_CALENDAR.value}() -> str\n"
            " - Reads your calendar and returns the end time of your next meeting.\n"
            " - Returns the end time in format 'YYYY-MM-DD HH:MM:SS'.\n"
            " - Returns 'No upcoming meetings' if none found.\n"
        ),
        stringify_rule=lambda args: f"{TinyAgentToolName.READ_CALENDAR.value}()",
    )
...
def get_create_note_tool(computer: Computer, notes_agent: NotesAgent) -> Tool:
    async def create_new_note(name: str, content: str, folder: str) -> str:
        # Check if content suggests using a template
        formatted_content = await notes_agent(name, content, mode=NotesMode.NEW)
        return computer.notes.create_note(name, formatted_content, folder)
    ...
    return Tool(
        ...
        description=(
            ...
            " - If the content suggests a specific type of note (handoff, admission, procedure, discharge), "
            "the appropriate template will be used automatically.\n"
...
def get_ask_sonar_tool(
    sonar_agent: SonarAgent
) -> Tool:
    async def get_ask_sonar(
        question: str
    ) -> str:
        return await sonar_agent(question)

    return Tool(
        name=TinyAgentToolName.ASK_SONAR.value,
        func=get_ask_sonar,
        description=(
            f"{TinyAgentToolName.ASK_SONAR.value}("
            "question: str, "
            ") -> str\n"
            " - Searches the internet to answer any question or gather any information. DON'T USE FOR LOCATION QUERIES \n"
            " - 'question' is required required.\n"
            " - 'question' should not be excessively long\n"
            " - 'question' should not be related to map or locations"
        ),
    )
...
APPS_TO_TOOL_NAMES = {
    App.CALENDAR: {
        TinyAgentToolName.CREATE_CALENDAR_EVENT,
        TinyAgentToolName.READ_CALENDAR
    },
    App.CONTACTS: {
    ...
    App.SONAR: {TinyAgentToolName.ASK_SONAR},
}
...
def get_tiny_agent_tools(
    ...
    # Sonar Tools
    if TinyAgentToolName.ASK_SONAR in tool_names:
        tools.append(get_ask_sonar_tool(sonar_agent))
    # Calendar reading tool
    if TinyAgentToolName.READ_CALENDAR in tool_names:
        tools.append(get_read_calendar_tool(computer))
